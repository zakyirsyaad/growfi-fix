...
import { buyMarketplaceListing } from "@/lib/game/service";
...
export async function POST(request: Request) {
  try {
    ...
    await rateLimit(`market-buy:${user.id}`, 30, 60_000);
    const input = await parseJson(request, listingIdSchema);
    return ok(await buyMarketplaceListing(user.id, input.listingId));
  } catch (error) {
