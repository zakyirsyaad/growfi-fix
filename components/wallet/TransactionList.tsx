    ...
    <Card>
      ...
      <div className="space-y-2">
        {transactions.length === 0 ? (
          <div className="text-sm font-bold text-leaf-800">No transactions yet.</div>
        ) : null}
        {transactions.map((tx) => (
          <div
            key={tx.id}
          ...
          >
            <div>
              <div className="font-black">{tx.type.toLowerCase().replaceAll("_", " ")}</div>
              <div className="text-xs font-bold text-leaf-700">
                {new Date(tx.createdAt).toLocaleString()}
              </div>
            </div>
            <div className="font-black">{tx.amount}</div>
            <div className="text-xs font-black uppercase text-leaf-700">{tx.status}</div>
            {tx.signature ? (
